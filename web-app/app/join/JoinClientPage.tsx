...
const JoinClientPage = ({ content }: JoinClientPageProps) => {
    ...
    return (
        <section
            ref={sectionRef}
            className="w-full mx-auto px-4 md:px-12 max-w-[1600px] my-32 md:my-40 flex flex-col gap-8"
        >
            <div className="flex flex-col md:flex-row w-full gap-20 xl:gap-40">
                {/* LEFT SIDE — TEXT & BUTTONS */}
                <div
                    className="w-full xl:w-2/5 h-auto relative"
                >
                    <h1 className="mb-10 md:mb-20">
                        {content.title}
                    </h1>
                    ...
                    >
                        ...
                        {content.buttons.map((button, index) => (
                            <Link
                                key={index}
                                className="flex flex-row gap-2 cursor-pointer items-center border-2 bg-charcoal border-cloud-white/20 hover:border-cloud-white/40 px-4 py-2 rounded-lg w-fit text-cloud-white transition duration-200"
