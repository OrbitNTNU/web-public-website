...
const JoinClientPage = ({ content, joinPage, teamInfo }: JoinClientPageProps) => {
 ...
 const handleGroupToggle = (group: string) => {
  ...
  const filteredPositions = selectedGroups.length > 0
    ? joinPage.components.filter(position => {
      const team = teamInfo.find(team => Number(team.teamID) === Number(position.team));
  ...
  return (
    <div className="w-full relative max-w-[2000px] mx-auto gap-0 md:gap-20 my-24 flex flex-col">
      ...
      <div ref={contentRef} className="px-4 md:px-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 justify-center gap-4 sm:gap-y-12">
