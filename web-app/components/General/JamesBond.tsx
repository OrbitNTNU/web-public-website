...
export default function JamesBond() {
  ...
  const cubesatX = useTransform(
    cubesatAngle,
    (a) => Math.cos(a) * radius.get(),
  );
  const cubesatY = useTransform(
    cubesatAngle,
    (a) => Math.sin(a) * radius.get(),
  );

  const studentAngle = useTransform(angle, (a) => a + 1.25);
  const studentX = useTransform(
    studentAngle,
    (a) => Math.cos(a) * radius.get(),
  );
  const studentY = useTransform(
    studentAngle,
    (a) => Math.sin(a) * radius.get(),
  );
