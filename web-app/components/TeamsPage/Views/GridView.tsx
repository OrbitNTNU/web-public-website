...
const GridView = ({
  ...
  const groupsWithSearch = Array.from(
    new Set(filteredTeamsData.map((team) => team.group)),
  );
  ...
  return (
    <div className="mb-40 space-y-12 px-4 md:px-12">
      {groupsWithSearch
              ...
              >
                {teamsInGroup.map((team, index) => {
                  return (
                    <motion.div
                      key={team.teamID}
                      className={`relative break-inside-avoid ${index !== 0 ? "mt-8" : ""} cursor-pointer hover:scale-105 transition duration-300 ease-in-out border-2 rounded-xl p-8 flex flex-col`}
                      variants={{
                        hidden: { opacity: 0 },
                        visible: {
                          opacity: 1,
