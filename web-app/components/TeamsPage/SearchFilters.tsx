        ...
        const options =
          filter === "Team"
            ...
            : filter === "Group"
              ? Array.from(
                  new Set(
                    teamsData.map((team) => team.group.split("_").join(" ")),
                  ),
                )
              : Array.from(
                  new Set(
                    teamsData.flatMap((team) =>
                      team.members.map((member) => member.privilege),
                    ),
                  ),
                );
        ...
        return (
          <div key={filter} className="relative">
            <button
              ...
              onClick={() =>
                setOpenDropdowns(
                  openDropdowns.map((open, i) => (i === idx ? !open : open)),
                )
              }
            >
              <span>{filter}</span>
            ...
            <div
              ...
              style={{
                maxHeight: openDropdowns[idx]
                  ? `${options.length * 2.5}rem`
                  : "0",
            ...
            {options.length > 0 && (
              <ul
                ...
                style={{
                  ...
                  height: openDropdowns[idx] ? `0` : "1.5rem",
              ...
              >
                {options
                  .filter((_, idx2) =>
                    selectedFilters[filter]?.includes(options[idx2]),
                  )
                  .map((option) => (
                    <li
                      key={option}
