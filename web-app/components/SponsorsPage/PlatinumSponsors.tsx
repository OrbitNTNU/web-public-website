import { PlatinumSponsor } from "@/sanity/types/sponsorsPage";
...
const PlatinumSponsors = ({ sponsors }: PlatinumSponsorsProps) => {
  ...
  return (
    <div className="flex items-center flex-col gap-20">
      <h1>{`Platinum sponsor` + (sponsors.length > 1 ? "s" : "")}</h1>
      <section className="w-full px-4 md:px-12 mx-auto flex flex-row justify-center flex-wrap">
        {sponsors.map((sponsor) => (
          <div
            key={sponsor._key}
            className="w-full md:w-1/3 justify-start items-center flex flex-col gap-8 p-8"
          >
            <Image
