...
const Statistics = () => {
  ...
  const [memberDistributionInTeams, setMemberDistributionInTeams] =
    useState<MemberDistributionInTeams | null>(null);
  const [studies, setStudies] = useState<Record<string, number> | null>(null);
  const [studyLevelDistribution, setStudyLevelDistribution] =
    useState<StudyLevelDistributionStatistics | null>(null);
  const [programStatistics, setProgramStatistics] =
    useState<FieldsOfEducationStatistics | null>(null);
  ...
  return (
    <div className="flex flex-col gap-20 md:gap-40 w-full mx-auto px-4 md:px-12">
      <MemberDistribution data={memberDistributionInTeams} />
      <Studies data={studies} />
      <StudyLevelDistribution data={studyLevelDistribution} />
      <FieldsOfEducation data={programStatistics} />
    </div>
  );
};
